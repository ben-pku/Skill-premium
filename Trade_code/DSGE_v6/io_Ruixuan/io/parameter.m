...
%PARAMETER get the parameters
    ...
    % production 
    pa.mu_K = [0.3 0.2]; % capital share
    pa.mu_L = [0.2 0.3]; % unskilled labor share
    pa.mu_m1 = [0.25, 0.25];
    pa.mu_m2 = [0.25, 0.25];
...
%     pa.D_L = temp + pa.D_L;
    ...
    % population
    pa.L = [ 6  ; 4  ;  2  ]; %(linspace(6, 2, pa.num))' .* ones(pa.num, 1); % t=0 L
    pa.H = [ 0.07  ;  0.1 ;  4 ];%(linspace(0.1 , 0.5, pa.num))' .* ones(pa.num, 1); % t=0 H
    
   
    % capital
    pa.k0 = [ 2  ;  2.5 ; 3 ] ;%(linspace(1, 1.5, pa.num))' .* ones(pa.num, 1);
