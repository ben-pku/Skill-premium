...
for iter = 1: maxit
    ...
    % a solve E, I; 
    if t == 1
        Q.E(:, t) = Q.sigma(:, t) .* p.r0 .* p.r_h(:, t) .* Q.k(:, t);
        Q.I(:, t) = (1- Q.sigma(:, t) ).* p.r0 .* p.r_h(:, t) .* Q.k(:, t);
    else
        Q.E(:, t) = Q.sigma(:, t) .* p.r(:, t-1) .* p.r_h(:, t) .* Q.k(:, t);
        Q.I(:, t) = (1- Q.sigma(:, t) ).* p.r(:,t-1) .* p.r_h(:, t) .* Q.k(:, t);
    end
    ...
    % c expenditure share: country * sec
...
% f solve the k t+1
if t < pa.T+1
    Q.k(:, t+1) = Q.I(:,t) ./ p.p2(:, t) + (1-pa.delta) * Q.k(:, t);
end
