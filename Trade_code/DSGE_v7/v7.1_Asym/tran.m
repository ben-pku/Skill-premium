...
for iter = 1: maxit
    ...
    for t = 1 : pa.T+1
        if t == 1
            numer = repmat(Q.u_Hh(:, t+1)', pa.num, 1 ) ./ pa.kappa_Hh(:,:, t)'.^(1/pa.rho);
            denom = sum(pa.D_H .* numer ,2 ); % num * 1
            fl.D_Hh( :, :, t ) = numer ./ repmat(denom, 1, pa.num);
            fl.D_H( :, :, t ) = fl.D_Hh( :, :, t ) .* pa.D_H;
        elseif t <= pa.T
            numer = repmat(Q.u_Hh(:, t+1)', pa.num, 1 ) ./ pa.kappa_Hh(:,:, t)'.^(1/pa.rho);
            denom = sum(fl.D_H(:, :, t-1) .* numer ,2 ); % num * 1
            fl.D_Hh( :, :, t ) = numer ./ repmat(denom, 1, pa.num);
            fl.D_H( :, :, t) = fl.D_Hh( :, :, t ) .* fl.D_H( :, :, t-1);
        else
            numer = repmat(ones(pa.num,1)', pa.num, 1 ) ./ pa.kappa_Hh(:,:, t)'.^(1/pa.rho);
            denom = sum(fl.D_H(:, :, t-1) .* numer ,2 ); % num * 1
            fl.D_Hh( :, :, t ) = numer ./ repmat(denom, 1, pa.num);
            fl.D_H( :, :, t) = fl.D_Hh( :, :, t ) .* fl.D_H( :, :, t-1);
        end
       ...
       if t == 1
            numer = repmat(Q.u_Lh(:, t+1)', pa.num+1, 1 ) ./ pa.kappa_Lh(:,:, t)'.^(1/pa.rho);
            denom = sum(pa.D_L .* numer ,2 ); % num+1 * 1
            fl.D_Lh( :, :, t ) = numer ./ repmat(denom, 1, pa.num+1);
            fl.D_L( :, :, t ) = fl.D_Lh( :, :, t ) .* pa.D_L;
        elseif t<= pa.T
            numer = repmat(Q.u_Lh(:, t+1)', pa.num+1, 1 ) ./ pa.kappa_Lh(:,:, t)'.^(1/pa.rho);
            denom = sum(fl.D_L(:, :, t-1) .* numer ,2 ); % num+1 * 1
            fl.D_Lh( :, :, t ) = numer ./ repmat(denom, 1, pa.num+1);
            fl.D_L( :, :, t) = fl.D_Lh( :, :, t ) .* fl.D_L( :, :, t-1);
        else
            numer = repmat(ones(pa.num+1,1)', pa.num+1, 1 ) ./ pa.kappa_Lh(:,:, t)'.^(1/pa.rho);
            denom = sum(fl.D_L(:, :, t-1) .* numer ,2 ); %  num+1 * 1
            fl.D_Lh( :, :, t ) = numer ./ repmat(denom, 1, pa.num+1);
            fl.D_L( :, :, t) = fl.D_Lh( :, :, t ) .* fl.D_L( :, :, t-1);
        end
    ...
    p.w_Lh = ones(pa.num+1 ,pa.T+1) ; % guess the w_Lh
    p.r_h = ones(pa.num,pa.T+1)  ; % guess the r_h
    p.w_H = NaN(pa.num,pa.T+1) ;
    p.w_L = NaN(pa.num+1,pa.T+1) ;
...
%     fl.S1_h = NaN(pa.num, pa.num, pa.T+1);
    ...
    Q.oL = NaN(pa.num+1, pa.sec, pa.T+1); % expenditure share of lowskill
...
% 6 update u_Hh, u_Lh
    newu_Hh = [NaN( pa.num, pa.T), ones(pa.num, 1)]; % updated u hat
    newu_Lh = [NaN( pa.num+1, pa.T), ones(pa.num+1, 1)] ; % updated u hat
    % solve C_h 
    omega2H_h = NaN(pa.num, pa.T+1);
    omega2L_h = NaN(pa.num+1, pa.T+1);
    for t = 1: pa.T+1
        if t == 1
    ...
    C_Hh = p.w_Hh ./ p.p_h2 .* omega2H_h.^(1/(1-pa.sigma)); % num*t
    C_Lh = p.w_Lh ./ [p.p_h2(1, :) ; p.p_h2 ].* omega2L_h.^(1/(1-pa.sigma)); % num+1 *t
    
    for t = pa.T : -1: 1
        if t ==1 
            eleH = pa.D_H .* repmat(newu_Hh(: , t+1)' , pa.num, 1 ) ./ ( pa.kappa_Hh( :, :, t)'.^(1/pa.rho));
            eleL  = pa.D_L .* repmat(newu_Lh(: , t+1)', pa.num+1, 1 ) ./ (pa.kappa_Lh(:, :, t)'.^(1/pa.rho)); 
        else
            eleH = fl.D_H(:, :, t-1) .* repmat(newu_Hh(: , t+1)' , pa.num, 1 ) ./ ( pa.kappa_Hh( :, :, t)'.^(1/pa.rho));
            eleL  = fl.D_L(:, :, t-1) .* repmat(newu_Lh(: , t+1)', pa.num+1, 1 ) ./ ( pa.kappa_Lh( :, :, t)'.^(1/pa.rho));
        end
