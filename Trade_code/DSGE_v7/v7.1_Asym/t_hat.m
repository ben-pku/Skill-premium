...
%  
    % unit cost
    p.uc_h1(:, t) =  [p.w_Lh(1, t);  p.w_Lh(3:pa.num+1, t) ]; % num * 1
    p.uc_h2(:, t) = p.r_h(:, t).^pa.mu_K(2) .* p.w_Lh(2:pa.num+1, t).^pa.mu_L(2) .* p.w_Hh(:, t).^pa.mu_H(2);
    p.uc_h3(:, t) = p.r_h(:, t).^pa.mu_K(3) .* p.w_Lh(2:pa.num+1, t).^pa.mu_L(3) .* p.w_Hh(:, t).^pa.mu_H(3);
    p.uc_h4(:, t) = p.r_h(:, t).^pa.mu_K(4) .* p.w_Lh(2:pa.num+1, t).^pa.mu_L(4) .* p.w_Hh(:, t).^pa.mu_H(4);
...
%     fl.S1(:,:, t) = numer1 ./ (p.p_h1(:,t).^(-pa.theta(1)));
    p.p_h1(:, t) =p.uc_h1(:, t)./pa.A_h(:, t) ; % num * 1
