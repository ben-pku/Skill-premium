...
for iter = 1: maxit
   ...
   Q.I(:, t) = p.p2(:, t) .* (Q.k(:,t+1) - (1-pa.delta)*Q.k(:, t));

   %  expenditure share: country * sec
    Q.oL(:, :, t) = share([p.p1(1,t) ; p.p1(:, t)], [p.p2(1,t);p.p2(:, t)], [p.p3(1,t);p.p3(:, t)], [p.p4(1,t) ;p.p4(:, t)], p.w_L(:, t), pa, pa.num+1); % low worker
    Q.oH(:, :, t) = share(p.p1(:, t), p.p2(:, t), p.p3(:, t), p.p4(:, t), p.w_H(:, t), pa,pa.num); % high worker
    Q.oK(:, :, t) = share(p.p1(:, t), p.p2(:, t), p.p3(:, t), p.p4(:, t), Q.E(:, t),  pa, pa.num); % landlord
...
% % update R(t)
% if t == 1
%     p.R(:, t) = 1 - pa.delta + p.r_h(: , t) ./ (   p.p_h2(:,t).^pa.gamma(2) ) .* ( p.R0 -1+pa.delta);
% else
%     p.R(:, t) = 1 - pa.delta + p.r_h(: , t) ./ (  p.p_h2(:,t).^pa.gamma(2) ) .* ( p.R(:, t-1 ) -1+pa.delta);
% end
%  if t < pa.T+1
%     Q.k(:, t+1) = (1 - Q.sigma(:, t)) .* p.R(:, t) .* Q.k(:, t);
%  end
