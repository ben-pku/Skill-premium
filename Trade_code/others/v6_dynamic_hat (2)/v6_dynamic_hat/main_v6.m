...
while dif > 1e-3
    ...
    [w_o_0,r_u_0] = initial(pa.S_0,pa.l_0,pa.k_0,pa,N,J);
    ...
    % temporary system
    [w_o_hat(:,:,tt), r_u_hat(:,tt)] = temp_eq(L(:,:,tt), K(:,tt), pa.S_0, w_o_0, r_u_0, pa, N, O, J, tt);
    ...
    p_x_hat(:,tt) = prod((p_hat(:,:,tt)).^pa.gamma,2);
    ...
    for tt = 2:TT+1
        % temporary system
        [w_o_hat(:,:,tt), r_u_hat(:,tt)] = temp_eq(L(:,:,tt), K(:,tt), S(:,:,tt-1), w_o(:,:,tt-1), r_u(:,tt-1), pa, N, O, J, tt);
        ...
        % a.5 capital return
        R(:,tt) = 1 - pa.delta + r_u_hat(:,tt)./p_x_hat(:,tt) .* (R(:,tt-1) - 1 + pa.delta );
    ...
    for tt=linspace(TT,1,TT)
        if tt > 1
            for j = 1:O
                if j == 1
                    u = u_hat(:,:,tt+1)';
                    a = (pa.b_hat(:,tt).*w_o_hat(:,j,tt)./p_x_hat(:,tt)).^(pa.beta./pa.rho);
